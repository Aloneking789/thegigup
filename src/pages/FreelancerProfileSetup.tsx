...
// import { FreelancerProfile } from "@/lib/api/types";
...
const FreelancerProfileSetup = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        setIsLoading(true);
        const existingProfile = await freelancerService.getProfile();
        ...
        if (existingProfile) {
          ...
          setProfileData(prev => ({
            ...
            role: existingProfile.title || "",
            bio: existingProfile.overview || existingProfile.bio || "",
            skills: Array.isArray(existingProfile.skills) 
              ? existingProfile.skills 
              : (typeof existingProfile.skills === 'string' ? existingProfile.skills.split(',').map(s => s.trim()) : []),
            hourlyRate: existingProfile.hourlyRate?.toString() || "",
            languages: existingProfile.languages || [],
            github: existingProfile.githubUrl || "",
            linkedin: existingProfile.linkedinUrl || "",
            profilePic: existingProfile.profilePicture || "",
            // Map portfolio to projects
            projects: existingProfile.portfolio?.map((item, index) => ({
              id: index + 1,
              title: item.title || "",
            ...
            // Map experience array to work experience format if available
            workExperience: existingProfile.experience && typeof existingProfile.experience === 'string' 
              ? [{
                  id: 1,
                  company: "",
                  title: "",
                  startDate: "",
                  endDate: "",
                  description: existingProfile.experience
                }]
              : [{
                  id: 1,
                  company: "",
  ...
  const handleSubmit = async () => {
    try {
      ...
      const apiData = {
        ...
        portfolioUrl: "", // Add portfolio URL if needed
        // Map projects to portfolio format
        portfolio: profileData.projects.filter(p => p.title).map(project => ({
          title: project.title,
          description: project.description,
          projectUrl: project.link,
          imageUrl: project.image,
          technologies: [] // Extract from project description if needed
        })),
        experience: profileData.workExperience[0]?.description || "",
            ...
            <div className="space-y-4">              <div className="space-y-2">
              ...
              {profileData.category && (
                <div className="space-y-2">
                  ...
                  <Select value={profileData.subcategory} onValueChange={(value) => handleInputChange("subcategory", value)}>
                    <SelectTrigger className="border-gray-200 focus:border-blue-500">
